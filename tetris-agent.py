...
MAX_GAMES = 5
explore_change = 0.1
weights = [-84.3601, -9.5186, -4.5711, -1.5503] # Initial weight vector
...
def get_parameters(board):
    ...
    for i in diffs:
        diff_sum += abs(i)
    return height_sum, diff_sum, max_height, holes #,time_alive
...
def get_expected_score(test_board, weights):
    ...
    D = weights[3]
    #E = weights[4]
    test_score = float(A * height_sum + B * diff_sum + C * max_height + D * holes )#E * time_alive )
    return test_score
...
def simulate_board(test_board, test_piece, move):
    ...
    reference_holes = get_parameters(test_board)[3]
    ...
    height_sum, diff_sum, max_height, holes  = get_parameters(test_board)
    one_step_reward = 5 * (test_lines_removed * test_lines_removed) - ( height_sum - reference_height) - (holes - reference_holes) *15 + (test_lines_removed - 3) *5
...
if __name__ == '__main__':
    ...
    while True:  # game loop
        ...
        print("Game Number ", games_completed, " achieved a score of: ", newScore,"weights:", weights)
        ...
        if games_completed >= MAX_GAMES:
            ...
            # f.close()
            print(sum(scoreArray)/len(scoreArray))
            plt.figure(1)
            plt.subplot(211)
